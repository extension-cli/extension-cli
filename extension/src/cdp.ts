...
export async function ensureAttached(tabId: number, aggressiveRetry: boolean = false): Promise<void> {
  ...
  // Normal commands: 2 retries, 500ms delay
  // Aggressive mode: 5 retries, 1500ms delay (tolerates extension interference)
  ...
  for (let attempt = 1; attempt <= MAX_ATTACH_RETRIES; attempt++) {
    try {
      ...
      if (attempt < MAX_ATTACH_RETRIES) {
        console.warn(`[extension-cli] attach attempt ${attempt}/${MAX_ATTACH_RETRIES} failed: ${lastError}, retrying in ${RETRY_DELAY_MS}ms...`);
  ...
  if (lastError) {
    ...
    console.warn(`[extension-cli] attach failed for tab ${tabId}: url=${finalUrl}, windowId=${finalWindowId}, error=${lastError}`);
...
export async function evaluate(tabId: number, expression: string, aggressiveRetry: boolean = false): Promise<unknown> {
  ...
  // Normal: 2 retries. Aggressive mode: 3 retries (tolerates extension interference).
